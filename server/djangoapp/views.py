...
def logout_user(request):
    if request.method == "POST":
        ...
        logout(request)
        return JsonResponse(
            {"userName": username, "status": "Logged out successfully"})
...
def get_dealer_reviews(request, dealer_id):
    if dealer_id:
        ...
        # Debugging print
        print(f"TEST API Response for dealer {dealer_id}: {reviews}")
        ...
        if not reviews:  # Empty list (no reviews found)
            return JsonResponse(
                {"status": 200, "reviews": [], "message": "No reviews available for this dealer."})

        valid_reviews = []
        for review_detail in reviews:
            if isinstance(review_detail, dict):  # Ensure it's a dictionary
                # Debugging print
                print(f"Processing review: {review_detail.get('review', '')}")
                ...
                if response is not None:
                    ...
                    # Set a default value if the response is None
...
def get_dealer_details(request, dealer_id):
    if (dealer_id):
        ...
        # Log the response
        print(f"API Response for dealer {dealer_id}: {dealership}")
